...
class TestMainBlackInteractiveShell : public TestInternal
{
 protected:
  void SetUp() override
  {
    ...
    //options->base.in = d_sin.get();
    //options->base.out = d_sout.get();
    options->base.inputLanguage = language::input::LANG_CVC;
    //d_solver.reset(new cvc5::api::Solver(std::move(options)));
